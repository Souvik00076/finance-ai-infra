...
def main():
    ...
    if not (args.github_url.startswith('https://github.com/') or
        ...
        sys.exit(1)
    target_dir = args.directory if args.directory else extract_repo_name(
        args.github_url)
    ...
    if repo_exists(target_dir):
        if args.force:
            print(f"Directory '{
                  target_dir}' exists but --force flag is set. Proceeding with clone...")
            shutil.rmtree(target_dir)
        else:
            print(f"Repository '{
                  target_dir}' already exists in current directory.")

    success = clone_repository(
        args.github_url, target_dir if args.directory else None, target_branch)
    if success:
        print(f"Repository cloned to: {os.path.abspath(target_dir)}")
    if args.directory:
        success = spin_up_docker_compose(
            True if args.force else False, args.directory)
    else:
